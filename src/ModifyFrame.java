	...
	ModifyFrame(){
		...
		btnSave.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ae){
				...
				if(id<=9999){
					JOptionPane.showMessageDialog(new JDialog()," Invalid id.ID should be of 5 digits ");
					txtID.setText("");
					txtID.requestFocus();
					return;
				}
				...
				try {
					...
					String q="update emp set name=?,department=?,salary=? where id=?";
					PreparedStatement pst=con.prepareStatement(q);
					pst.setInt(4,id); //this becomes 4 as the parameter ie id is accepted nd in the query
